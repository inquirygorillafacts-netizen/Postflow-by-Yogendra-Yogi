...
export async function createProcessedImage(imageUrl, settings, logoUrl) {
  const {
    ...
    aspect,
    naturalAspect,
    logos = [], // new array of logo objects
    showLogo = false,
    logoOpacity = 70,
    logoSizePercent = 30,
  ...
  // Compile the list of logos to draw
  let logosToDraw = [...logos];
  // Backwards compatibility for single logo state
  if (logosToDraw.length === 0 && showLogo && logoUrl) {
    logosToDraw.push({
      url: logoUrl,
      opacity: logoOpacity,
      size: logoSizePercent,
      position: logoPosPercent
    });
  }

  // Overlay all logos
  for (const logo of logosToDraw) {
    if (!logo.url) continue;
    try {
      const logoImage = await createImage(logo.url);

      const logoW = (canvas.width * logo.size) / 100;
      const logoH = logoW * (logoImage.naturalHeight / logoImage.naturalWidth);

      const logoX = (canvas.width * logo.position.x) / 100 - logoW / 2;
      const logoY = (canvas.height * logo.position.y) / 100 - logoH / 2;
